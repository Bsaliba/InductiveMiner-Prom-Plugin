...
public class Miner2 {
	...
	public static Node mineNode(IMLog log, ProcessTree tree, MiningParameters parameters) {
		...
		if (cut != null && cut.isValid()) {
			...
			return attachStatistics(newNode, logInfo);
	...
	public static Node attachStatistics(Node node, IMLogInfo logInfo) {
		System.out.println("attach statistics");
		PropertyNumberOfTracesRepresented property1 = new PropertyNumberOfTracesRepresented();
		try {
			node.setIndependentProperty(property1, new Integer((int) logInfo.getNumberOfTraces()));
